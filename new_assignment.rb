def new_clean_table(client) 
	...
	begin
	 	...
	 	client.query("CREATE TABLE montana_public_district_report_card__uniq_dist_henri (
 ...
 insertion = <<~SQL
 	insert IGNORE into montana_public_district_report_card__uniq_dist_henri 
    ...
    clean = <<~SQL
    SELECT id,name 
    from montana_public_district_report_card__uniq_dist_henri 
    SQL
    
    new_clean = client.query(clean).to_a
    new_clean.each do |row|
      clean_name = row['name'].gsub("Elem", "Elementary"
        ).gsub(/\bH ?S\b/, "High School"
        ).gsub(/K-12( Schools)?/, "Public School").gsub(/\b(\w+) \1/i, '\1') + " District"


      

      client.query("UPDATE montana_public_district_report_card__uniq_dist_henri 
